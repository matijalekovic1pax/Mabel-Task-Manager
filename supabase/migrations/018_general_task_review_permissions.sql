-- ============================================================
-- 018_general_task_review_permissions.sql
-- Prevent assignees from approving or sending back their own
-- general-task review submissions, even when they are admins.
-- ============================================================
...
BEGIN
  ...
  v_is_assigner := EXISTS (
    SELECT 1 FROM public.task_assignees
    WHERE task_id = p_task_id AND assigned_by = v_actor_id
  );

  IF NOT (v_is_admin OR v_is_creator OR v_is_assignee OR v_is_assigner) THEN
  ...
  CASE v_action
    ...
    WHEN 'approve_close' THEN
      ...
      END IF;
      IF NOT (v_is_creator OR v_is_assigner) THEN
        RAISE EXCEPTION 'Only the task creator or assigner can approve and close';
      END IF;
      IF v_is_assignee THEN
        RAISE EXCEPTION 'Assignees cannot approve and close their own submitted work';
      END IF;
      v_to_status := 'done';
    ...
    WHEN 'send_back' THEN
      ...
      END IF;
      IF NOT (v_is_creator OR v_is_assigner) THEN
        RAISE EXCEPTION 'Only the task creator or assigner can send the task back';
      END IF;
      IF v_is_assignee THEN
        RAISE EXCEPTION 'Assignees cannot review their own submitted work';
      END IF;
      IF v_note IS NULL THEN
  ...
  -- Closed by assignee directly without review.
  IF v_action = 'complete' AND v_actor_id <> v_task.submitted_by THEN
    INSERT INTO public.notifications (recipient_id, type, title, message, task_id)
    VALUES (
  ...
  -- Creator reopened a done task: notify assignees it is back in play.
